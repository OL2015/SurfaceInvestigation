...
class RawDataBuilder(object):
    ...
    def __init__(self, W, H, Micron_Per_Pixel, lambda0, bandwidth, piezoStep = 0.314,  **kwargs):
        ...
        self.piezoStepNoise = piezoStep/64
        self.I0 = 100
        self.I0fiber = 95
        self.rfl = 0.99
    ...
    def rawTwoSphere(self,R, piezoStart , piezoEnd , whitlight=True, fi0=0, z0=0, piezoRange =None, frameCallback=None):
        """
        ...
        """
        R2 = R*R
        x1 = (np.arange(0, self.W/2, 1) - self.W/2)*self.Micron_Per_Pixel + R   #   pix
        x2 = (np.arange(0,self.W/2,1))*self.Micron_Per_Pixel - R;
        y = (np.arange(0, self.H, 1) - self.H/2)*self.Micron_Per_Pixel    # pix
        ...
        Dst2 = (xx ** 2 + yy ** 2) 
        r=np.sqrt(Dst2) 
        Dst2_temp = Dst2 * ( r < R  ) + 0*( r >= R  )
        dZ0 = (z0 + ((R - np.sqrt(R2 - Dst2_temp)) * ( r < R)))
        ...
        if piezoRange != None:
            ...
            piezoRange = piezoRange + np.random.normal(loc=0, scale=self.piezoStepNoise, size=zN) #camera noise
        for z in piezoRange:  #micron

            dZ = z + dZ0 # microns
            q2 = (dZ / self.Lc) ** 2
            npI = self.I0 * ( np.sqrt(1 - Dst2_temp / R2)) * (r> R ) +  self.I0fiber * (r <= R )
            if whitlight:
            ...
            if self.cameraNoiseSigma>0:
                npInt = npInt + np.random.normal(loc=0, scale=self.cameraNoiseSigma, size=(self.H,self.W)) #camera noise
